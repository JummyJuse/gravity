...
using namespace std;
int res_x = 15, res_y = 14; //caps at around 89, 50 (depending on the termanal window size)
...
void renderScreen() {
	...
	for (int i = 0; i < res_y; ++i) {
		...
		for (int p = 0; p < res_x; ++p) {
			...
			else if (screen[i * res_x + p] == length) {
				if ((pkey == 'w') or (pkey == 's')) {
					cout << "oo";
				}
				else {
...
void graphPoints() {
	...
	if ((screen[(yr)*res_x + xr] > 0) and (pkey)) {
		alive = false;
	}
